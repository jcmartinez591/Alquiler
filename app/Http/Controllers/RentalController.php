...
class RentalController extends Controller
{
    ...
    public function index(){
        ...
        foreach ($rentals as $rental) {
            $date2 = date_create_from_format("Y-m-d H:i:s", $rental->return_date);
            if(($date2 < $date) && ($rental->status != 'Disponible')){
                $rental->update([
                    'status' =>'Reservado'
                ]);
            }
        ...
        return view('rentals.index', compact('rentals', 'date'));
